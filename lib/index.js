...
*/
const OLD_BUILTIN_DEFAULT_PRICE_ENTRIES = [
	{
		model: "deepseek-v4-flash",
		cacheHitInputPricePerMillion: .02,
		cacheMissInputPricePerMillion: 1,
...
*/
function isLegacyBuiltinDefaultPrices(prices) {
...
function priceEntriesEqualNormalized(a, b) {
	...
	for (const [key, count] of aMap) if (bMap.get(key) !== count) return false;
	return true;
}
...
function bandForMinute(schedule, minuteOfDay) {
	for (const window of schedule.windows) if (isInsideWindow(minuteOfDay, window)) return {
		bandId: window.bandId ?? window.id,
		window
	};
	return {
...
*/
function offPeakSpans(schedule) {
	const covered = [];
	for (const window of schedule.windows) for (const interval of windowIntervals(window)) covered.push(interval);
	covered.sort((a, b) => a[0] - b[0]);
	const spans = [];
	let cursor = 0;
	for (const [start, end] of covered) {
		if (start > cursor) spans.push({
			start: cursor,
			end: start
		});
		if (end > cursor) cursor = end;
	}
	if (cursor < 1440) spans.push({
		start: cursor,
		end: 1440
	});
	return spans;
...
function offPeakSpanForMinute(schedule, minuteOfDay) {
...
function validatePricingScheduleSet(set) {
	...
	for (const schedule of set.schedules) {
		...
		const bandIds = /* @__PURE__ */ new Set();
		const intervals = [];
		...
		for (const model of schedule.models) {
			...
			for (const [bandId, rates] of Object.entries(model.ratesByBand)) {
				if (bandId !== "off-peak" && !bandIds.has(bandId)) throw new Error(`deepseek-usage: schedule ${schedule.id} model ${model.model} references unknown band "${bandId}"`);
...
*/
const DEEPSEEK_2026_08_17_SCHEDULE = {
	...
	models: [{
		...
		ratesByBand: {
			...
			"off-peak": {
				cacheHitInputPricePerMillion: .05,
		...
		ratesByBand: {
			peak: {
				cacheHitInputPricePerMillion: .3,
				cacheMissInputPricePerMillion: 9,
				outputPricePerMillion: 27
			},
			"off-peak": {
				cacheHitInputPricePerMillion: .15,
				cacheMissInputPricePerMillion: 4.5,
				outputPricePerMillion: 13.5
			}
		}
	}]
};
...
*/
const DEFAULT_SCHEDULES = [LEGACY_SCHEDULE, DEEPSEEK_2026_08_17_SCHEDULE];
...
function aggregateDayCost(schedules, rows) {
	...
	const bandShares = /* @__PURE__ */ new Map();
	for (const row of rows) {
		...
		total += cost;
		const share = bandShares.get(resolved.bandId) ?? {
			total: 0n,
			count: 0,
			cacheHit: 0,
			cacheMiss: 0,
...
function mapWireUsage(usage) {
	...
	return {
		inputTokens: nativeMiss !== void 0 ? nativeMiss : Math.max(0, usage.prompt_tokens - (cacheHit ?? 0)),
...
function resolvePricingSet(config) {
	...
	if (config.prices !== void 0 && config.prices.length > 0) {
		...
		if (isLegacyBuiltinDefaultPrices(config.prices)) return {
...
*/
function currentBandOf(set, nowMs) {
	const prepared = prepareScheduleSet(set.schedules);
	let active;
	for (const candidate of prepared) if (candidate.effectiveMs <= nowMs) active = candidate;
	else break;
...
//#endregion
export { ALL_DAY_WINDOW_ID, BALANCE_TIMEOUT_MS, BALANCE_URL, Config, DAY_TIMEZONE, DEEPSEEK_2026_08_17_SCHEDULE, DEEPSEEK_API_HOST, DEFAULT_DEEPSEEK_PROVIDER, DEFAULT_PRICE_ENTRIES, DEFAULT_SCHEDULES, LEGACY_SCHEDULE, OFF_PEAK_BAND_ID, OLD_BUILTIN_DEFAULT_PRICE_ENTRIES, USAGE_API_PREFIX, USAGE_SETTINGS_NAMESPACE, UsageStore, aggregateDayCost, apply, bandForMinute, bucketsFromTokenUsage, buildSchedulesFromPriceEntries, currentBandOf, dayKeyOf, dayRangeMs, dayRangeMsInTimezone, fetchBalance, formatMinuteOfDay, inject, isInsideWindow, isLegacyBuiltinDefaultPrices, makeUsageRoutes, mapWireUsage, minuteOfDayInTimezone, normalizeEffectiveFrom, offPeakSpanForMinute, offPeakSpans, prepareScheduleSet, priceEntriesEqual, resolveDeepseekEndpoint, resolvePriceEntry, resolvePricing, resolvePricingSet, sanitizeBalanceBody, validatePricingScheduleSet };
