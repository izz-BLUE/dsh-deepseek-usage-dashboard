...
import { aggregateDaily } from "./stats.js";
import { dayRangeMs } from "./day.js";
...
export class UsageStore {
     ...
     */
    migrateRequestTime() {
        const columns = this.db.prepare('PRAGMA table_info(usage_rows)').all();
    ...
    hasRow(sessionId, turn, step) {
        const row = this.db.prepare('SELECT 1 FROM usage_rows WHERE session_id = ? AND turn = ? AND step = ?').get(sessionId, turn, step);
        return row !== undefined;
    }
    /** Read every row in one Asia/Shanghai day's [start, end) range. */
    rowsInRange(startMs, endMs) {
        const rows = this.db.prepare('SELECT * FROM usage_rows WHERE time_ms >= ? AND time_ms < ? ORDER BY time_ms, session_id').all(startMs, endMs);
        return rows.map(rowFromSql);
    }
    /** Read every row ever stored (tests and trend scans). */
    allRows() {
        const rows = this.db.prepare('SELECT * FROM usage_rows ORDER BY time_ms, session_id').all();
        return rows.map(rowFromSql);
    }
    ...
    rowCount() {
        const row = this.db.prepare('SELECT COUNT(*) AS count FROM usage_rows').get();
        return row.count;
    }
    ...
    loadBalance() {
        ...
        try {
            ...
            return { snapshot: parsed, fetchedAtMs: row.fetched_at_ms };
        }
        catch {
            return undefined;
        }
