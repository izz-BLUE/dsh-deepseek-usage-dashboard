...
import { DEFAULT_SCHEDULES, aggregateDayCost, bandForMinute, buildSchedulesFromPriceEntries, formatMinuteOfDay, offPeakSpanForMinute, offPeakSpans, prepareScheduleSet, pricingSetsEqual, validatePricingScheduleSet, } from "./core/schedule.js";
 ...
 */
export function currentBandOf(set, nowMs) {
    const prepared = prepareScheduleSet(set.schedules);
    let active;
    for (const candidate of prepared) {
        if (candidate.effectiveMs <= nowMs)
            active = candidate;
        else
            break;
    }
