...
import { usageStore } from "../store-host.js";
...
export const DockLine = memo(function DockLine() {
    ...
    const snapshot = useSyncExternalStore((listener) => store?.subscribe(listener) ?? (() => undefined), () => store?.getSnapshot() ?? { data: null, error: null, loading: false, refreshing: false });
        ...
        : band.bandId === 'off-peak'
            ...
            : band.bandId === 'peak'
                ...
                : band.bandId === 'all-day'
                    ...
                    : band.bandId;
    return (_jsxs("div", { style: STYLE, title: tt('dock.todayTooltip'), children: [tt('dock.today', {
 ...
 */
export const DockLineEntry = memo(function DockLineEntry(_props) {
    return _jsx(DockLine, {});
});
