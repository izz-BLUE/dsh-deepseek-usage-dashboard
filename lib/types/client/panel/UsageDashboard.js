...
function TrendChart(props) {
    ...
    return (_jsx("div", { className: css.trend, "aria-label": "7-day token usage", children: props.data.map(day => (_jsxs("div", { className: day.date === latestDate ? `${css.trendCol} ${css.trendToday}` : css.trendCol, children: [_jsx("span", { className: css.trendValue, children: formatCompactCount(day.totalTokens) }), _jsx("div", { className: css.trendBarWrap, role: "meter", "aria-label": `${day.date}: ${formatCount(day.totalTokens)}`, "aria-valuemin": 0, "aria-valuemax": max, "aria-valuenow": day.totalTokens, title: `${day.date}: ${formatCount(day.totalTokens)}`, children: day.totalTokens > 0
                        ? (_jsx("div", { className: css.trendBar, style: { height: `${max === 0 ? 0 : Math.max(5, (day.totalTokens / max) * 100)}%` } }))
                        : _jsx("span", { className: css.trendZero }) }), _jsx("span", { className: css.trendLabel, children: day.date.slice(5) })] }, day.date))) }));
}
