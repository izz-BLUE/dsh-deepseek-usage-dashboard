...
import { DEFAULT_PRICE_ENTRIES, assertValidPriceEntry, costOfBuckets, priceEntriesEqual, resolvePriceEntry, type PriceEntry } from './core/pricing.ts'
import { recentDayKeys, dayRangeMs, dayKeyOf } from './core/day.ts'
import { formatMicro, sumMicro } from './core/money.ts'
...
import { makeUsageRoutes, type DayCostEstimate, type PriceTableMeta } from './host/routes.ts'
...
function resolvePrices(config: Config): PriceEntry[] {
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  const pricesOf = (): PriceEntry[] => resolvePrices(current())
  ...
  const estimateDayCost = (dayKey: string): DayCostEstimate => {
    if (store === undefined) return { total: '0', totalMicro: '0', currency: 'CNY' }
  ...
  const bumpPriceVersion = (previous: PriceEntry[]): void => {
