...
export function dayRangeMs(dayKey: string): { startMs: number; endMs: number } {
  return dayRangeMsInTimezone(dayKey, DAY_TIMEZONE)
}

/** The inclusive [start, end) epoch-millisecond range of one `timezone` day. */
export function dayRangeMsInTimezone(dayKey: string, timezone: string): { startMs: number; endMs: number } {
...
export function minuteOfDayInTimezone(epochMs: number, timezone: string): number {
  const clock = wallClockAt(epochMs, timezone)
...
export function recentDayKeys(todayKey: string, count: number): string[] {
  const keys: string[] = []
  let cursor = todayKey
