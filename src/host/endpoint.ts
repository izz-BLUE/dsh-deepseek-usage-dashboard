...
/** The llm-deepseek settings section shape (the fields we read). */
interface LlmDeepseekSection {
  baseURL?: string
  apiKeyEnv?: string
}
 ...
 */
export function resolveDeepseekEndpoint(ctx: Context, providerId: string): DeepseekEndpointFacts {
  let section: LlmDeepseekSection | undefined
  const settings = ctx.get('settings')
  if (settings !== undefined) {
    const raw = settings.get(settingsNamespace('llm-deepseek'))
    if (typeof raw === 'object' && raw !== null) section = raw as LlmDeepseekSection
  }
...
export function deepseekApiKeyRef(ctx: Context): string {
  const settings = ctx.get('settings')
  if (settings !== undefined) {
    const raw = settings.get(settingsNamespace('llm-deepseek'))
    if (typeof raw === 'object' && raw !== null) {
      const apiKeyEnv = (raw as LlmDeepseekSection).apiKeyEnv
